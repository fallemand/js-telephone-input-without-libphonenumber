...
function telephoneInputMask(field, parameters) {
    ...
    function changed(event) {
        ...
        var oldStart = field.selectionStart,
            ...
            value = applyMask();
        if (event && metadata.maskCharacters.indexOf(value.charAt(oldStart - 1)) > -1) {
            var charCode = (event.which) ? event.which : event.keyCode;
            if ([37, 39, 8].indexOf(charCode) === -1 || oldStart === 0) {
                oldStart = oldStart + 1;
                oldEnd = oldEnd + 1;
            }
        }
    ...
    function getMask(data) {
        ...
        var withZero = false;
        var cellphone = false;
        if (metadata.acceptZero && startsWithZero(dataCopy)) {
            dataCopy.shift();
            withZero = true;
        }
        ...
        if(withZero) {
            if(cellphone) {
                return metadata.masks.cellphone[cellphoneCodeLength].withZero[codeAreaDigits].split('')
            }
            else {
            ...
            if(cellphone) {
                return metadata.masks.cellphone[cellphoneCodeLength][codeAreaDigits].split('')
            }
            else {
    ...
    function startsWithCellphone(value) {
        var cellphoneLength = -1;
        if(metadata.cellphoneCodes) {
            for(var i = metadata.cellphoneCodesMaxLength; i >= metadata.cellphoneCodesMinLength; i-=1) {
                var aux = value.slice(0, i);
                if(metadata.cellphoneCodes.indexOf(aux.join('')) > -1) {
                    cellphoneLength = aux.length;
                    break;
                }
            }
        }
    ...
    function getMaxCharacters() {
        ...
        if(metadata.acceptZero && startsWithZero(value)) {
            value.shift();
            maxCharacters += 1;
        }
